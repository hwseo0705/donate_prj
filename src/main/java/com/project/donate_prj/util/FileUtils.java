...
public class FileUtils {
    ...
    // 1. 사용자가 파일을 업로드 했을 때, 새로운 파일명을 생성해서 반환하고
    //      해당 파일명으로 업로드하는 메서드
    // ex) 사용자가 상어.jpg를 올렸으면 이름을 저장하기 전에 중복없는 이름으로 바꿈

    /**
     * @param file       - 클라이언트가 업로드한 파일 정보
     * @param uploadPath - 서버의 업로드 루트 디렉토리 (E:/sl_Haewon_Seo/upload)
     * @return - 업로드가 완료된 새로운 파일의 이름
     */
    public static String uploadFile(MultipartFile file, String uploadPath) {

        // 중복이 없는 파일명으로 변경하기
        // ex) 상어.png -> dkfasfvtawetybrenjngfj-asfdvwefewfvsfevf-dfdjfld_상어.png
        String newFileName = UUID.randomUUID().toString() + "_" + file.getOriginalFilename();

        // 업로드 경로도 변경
        // E:/sl_Haewon_Seo/upload -> E:/sl_Haewon_Seo/upload/2022/08/01
        ...
        // 파일의 풀 경로 (directory path + file name)
        String fileFullPath = newUploadPath + File.separator + newFileName;

        // 풀 경로 - 루트 경로 문자열 생성
        // full-path => E:/sl_Haewon_Seo/upload/2022/08/01/dfsdjfksfdkjs_상어.jpg
        // res-path => /2022/08/01/dfsdjfksfdkjs_상어.jpg
        // uploadPath => E:/sl_Haewon_Seo/upload
    ...
    /**
     * 원본 업로드 경로를 바아서 일자별 폴더를 생성한 후 최종경로를 리턴
     *
    ...
    // 한자리수 월, 일 정보를 항상 2자리로 만들어조는 메서드
    private static String len2(int n) {
        return new DecimalFormat("00").format(n);
    }

    // 파일명을 받아서 확장자를 반환하는 함수
